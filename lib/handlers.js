...
handlers._tokens.get = function(data, cb){
    ...
    if(phone && password){
        crud.read('user', phone, function(err, data){
            if(err){
                return cb(404, {
                    "Error": "FCouldn't find the user data "
                });
            };
            const hashedPassword = helpers.hash(password);
            if(user.hashedPassword === hashedPassword){
                const tokenId = helpers.createdRandomString(20);
                ...
                return resizeBy.status(403, {"Error": "Password did not match the specified user"})
            }
        })
    } else {
        cb(400, {
